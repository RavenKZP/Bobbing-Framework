...
RayCastResult RayCast::Cast(RE::Actor* Actor, std::function<bool(RE::NiAVObject*)> const& evaluator, float raySize) {
    constexpr float PI_HALF = 1.5707963267948966f;
    RE::NiPoint3 downAngle(PI_HALF, 0.0f, 0.0f);
    RE::NiPoint3 ActorPosition = Actor->GetPosition();
    ActorPosition.z += 10;
    auto [ray_position, ray_object] = CastRay(Actor, downAngle, ActorPosition, evaluator, raySize);
